...
function authButton(user) {
  ...
  return user && user.isAuthenticated
    ? /* html */ `
    <button class="btn btn-small btn-outline-dark text-muted selectable me-2" onclick="app.AuthController.logout()"><i class="mdi mdi-logout f-16"></i></button>
  `
    : /* html */ `
    <button class="btn btn-dark selectable me-2" onclick="app.AuthController.login()">login</button>
  `
}
...
function avatarTemplate(account) {
  return account
    ...
    <div class="mr-2">
      ...
      <span class="mx-1 text-dark">${account.name}</span>
      </div>`
    : AuthService.loading
      ? /* html */ `
